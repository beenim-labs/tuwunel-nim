...
suite "Behavior coverage artifacts":
  test "report totals align with baseline inventory":
    ...
    check routeRuntime["summary"]["total_routes"].getInt() == baseline["totals"]["route_total"].getInt()
  ...
  test "threshold booleans are exposed for milestone gating":
    ...
    check routeRuntime["thresholds"]["has_runtime_dispatch"].kind == JBool
    check routeRuntime["thresholds"]["all_routes_runtime_implemented"].kind == JBool
